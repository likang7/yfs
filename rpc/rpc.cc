		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	std::map<unsigned int, std::list<reply_t> >::iterator replys = reply_window_.find(clt_nonce);

	std::list<reply_t>::iterator it = replys->second.begin();
	rpcstate_t state;
	for( ; it != replys->second.end(); it++){
		if(it->xid == xid){
			//xid from clt_nonce has already been reveived
	...
	//xid has not been received
	replys->second.push_front(reply_t(xid));
		...
		char *b, int sz)
{
	ScopedLock rwl(&reply_window_m_);
        // You fill this in for Lab 1.
	std::list<reply_t>& replys = reply_window_.find(clt_nonce)->second;
