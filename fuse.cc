                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    yfs_client::status ret = yfs->setattr(ino, info);
    if(ret != yfs_client::OK){
      fuse_reply_err(req, ENOENT);
      return;
    }
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  // Change the above "#if 0" to "#if 1", and your code goes here
    ...
    yfs_client::status ret;
    std::string buf;
    if((ret = yfs->readfile(inum, size, off, buf)) != yfs_client::OK){
        fuse_reply_err(req, ENOSYS);
        return;
    }
                 ...
                 struct fuse_file_info *fi)
{
  ...
  // Change the above line to "#if 1", and your code goes here
    ...
    if((ret = yfs->writefile(inum, off, buf, size)) != yfs_client::OK){
        fuse_reply_err(req, ENOSYS);
        return;
    }
    printf("write sucess\n");
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
    ...
    printf("helper\n");
    ret = yfs->create(iparent, name, mode, 0, ichild);
    printf("%d\n", ret);
    if (ret == yfs_client::OK) {
         struct stat st;
         e->ino = ichild;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // You fill this in for Lab 2
  printf("lookup %s\n", name);
    yfs_client::inum cinum;
    if(yfs->lookupinode(parent, name, cinum) == yfs_client::OK){
        struct stat st;
        if(getattr(cinum, st) == yfs_client::OK){
            e.attr = st;
            e.ino = cinum;
            found = true;
        }
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  if(yfs->readdir(inum, dirents) == yfs_client::OK){
    for(std::vector<yfs_client::dirent>::iterator it = dirents.begin(); 
        it != dirents.end(); it++){
